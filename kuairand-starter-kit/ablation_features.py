"""Bring in CWM's 13 feature fields to verify whether user-side features are useful in FM."""
...
def build(mode):
    """mode: 'base'=5 fields (current kit) / 'item'=only add item-side features / 'cwm13'=all 13 CWM fields"""
    ...
    def raw(x):
        ...
        if mode=='cwm13':            f += ue                                  # +6 user-side fields
        return f
...
for mode,desc in [('base','5 fields (current kit)'),('item','+4 item-side = 9 fields'),('cwm13','CWM full 13 fields')]:
    ...
    print(f"{desc:20s} ({nf:2d} fields) | test GAUC {g:.4f} | nDCG@5 {n5:.4f} | primary {pr:.4f} ± {sd:.4f}")
