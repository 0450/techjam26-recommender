"""
train_and_blend_all.py
-----------------------
Heterogeneous Neural Trainer & Blender.
1. Trains 5-seed Compact SENet DeepFM.
2. Trains 5-seed Low-Rank DCNv2.
3. Automatically finds optimal power-rank exponent and blend weights on Validation data.
4. Generates final blended submission file without manual .npy file handling.
...
def train_model(model_type, seeds, enc, aux_targets, num_features, device, epochs=12, batch_size=8192, patience=3):
    ...
    # Simple mean across 5 seeds for single model representation
    ...
    print(f"\n--> {model_type.upper()} 5-Seed Ensemble finished | Val Primary: {val_res['primary']:.4f} | Val GAUC: {val_res['GAUC']:.4f}")

    return model_val_ensemble, model_test_ensemble
...
def main():
    ...
    w1_candidates = np.linspace(0.1, 0.9, 17)  # Weights from 0.1 to 0.9
    ...
    print("\n" + "=" * 60)
    print("=== FINAL OPTIMAL BLEND RESULTS ===")
    print("=" * 60)
    print(f"  * SENet DeepFM Weight  : {best_w1:.2f}")
    print(f"  * Low-Rank DCNv2 Weight: {best_w2:.2f}")
    print(f"  * Optimal Power Exponent: {best_power:.1f}")
    print("-" * 60)
    ...
    # Step 4: Generate Blended Test Predictions
    ...
    output_filename = 'submission_heterogeneous_blend.csv'
    write_submission(output_filename, splits['test'], blended_test_preds)
    read_submission(output_filename, splits['test'])

    print(f"\nSaved final blended predictions to {output_filename}!")
